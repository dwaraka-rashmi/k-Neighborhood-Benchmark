 ...
 */
public class InitNeighborhoodScorerMapReduce {
    ...
    public static Boolean letterScorerMapReduceSetup(String input, String output)
        ...
        job.setJarByClass(InitNeighborhoodScorerMapReduce.class);
        ...
        if(jobStatus && job.isSuccessful()) {
            scoreLettersWriteToHDFS(conf);
            long endTime = System.nanoTime();
            bw.write("Letter Score total time,"+((endTime-startTime)/1000000)+"\n");
            bw.flush();
        }
        return jobStatus;
    }
    ...
    public static Boolean KNeighborhoodScorerMapReduceSetup(String k,String input, String output)
        ...
        if(dataCleanserIntoOverlappingBlocks(k,input)) {
            ...
            job.setJarByClass(InitNeighborhoodScorerMapReduce.class);
            job.setMapperClass(KNeighborMapper.class);
            ...
            if (jobStatus && job.isSuccessful()) {
                ...
                long endTime = System.nanoTime();
                BufferedWriter bw = new BufferedWriter(new FileWriter("output/benchmark.csv", true));
                bw.write("KNeighborhood Score total time," + ((endTime - startTime) / 1000000) + "\n");
                bw.flush();
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i = 1; i<=iterations ;i++) {
           ...
           if (letterScorerMapReduceSetup(args[2], "letter-count")) {
            ...
            long endTime = System.nanoTime();
            bw.write("total,"+((endTime-startTime)/1000000)+"\n");
            bw.flush();
        }
        bw.close();
    }
